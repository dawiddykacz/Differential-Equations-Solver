...
def run_all(learning_rate: float):
    ...
    for with_noise in [False]:
        for alpha in [0.9]: #0.1, 0.8, 0.85, 0.9, 0.95, 1
            task_repository.add_task(
                ExampleSecond2ProblemLossTaskWithWeight(alpha=alpha, alpha_lower=0.8,
                                                        with_noise=with_noise))
            for alpha_lower in [0.2, 0.15, 0.1, 0.05]:
                task_repository.add_task(
                    ExampleFirst2ProblemLossWithWeightTask(alpha=alpha, alpha_lower=alpha_lower,
                                                           with_noise=with_noise))
                task_repository.add_task(
    ...
    for with_noise in [True]:
        for alpha in [0.85, 0.9]: #0.1, 0.8, 0.85, 0.9, 0.95, 1
