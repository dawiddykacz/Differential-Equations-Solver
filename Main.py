...
def run_all(learning_rate: float):
    ...
    task_repository.add_task(
        ExampleSecond2ProblemLossTaskWithWeight(alpha=0.1, alpha_lower=0.85,
                                                with_noise=True))
    task_repository.add_task(
        ExampleFirst2ProblemLossWithWeightTask(alpha=0.1, alpha_lower=0.8,
                                               with_noise=True))
    task_repository.add_task(
        ExampleSecond2ProblemLossTaskWithWeight(alpha=0.1, alpha_lower=0.8,
                                                with_noise=True))
    for with_noise in [True]: # chcange to False
